...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ['walking'] = function(dt)
            ...
            if self.map:winner(self.map:tileAt(self.x + self.width, self.y )) or 
                self.map:winner(self.map:tileAt(self.x, self.y)) or 
                self.map:winner(self.map:tileAt(self.x, self.y + self.height)) then
                    self.dx = 0
                    self.state = 'winner'
            end
        end,
        ...
        ['winner'] = function()
            self.map.flagTimer = love.timer.getTime()
            if self.y < self.map.mapHeightPixels / 2 - self.map.tileHeight - self.height then
                self.dy = JUMP_VELOCITY / 4
                self.animation = self.animations['falling']
                self.sounds['win']:play(2)
            else
                self.dy = 0
                self.y = self.map.mapHeightPixels / 2 - self.map.tileHeight - self.height
                self.animation = self.animations['idle']
            end
            
            if self.x <= self.map.mapWidthPixels - self.map.tileWidth * 4 and self.x >= self.map.mapWidthPixels - self.map.tileWidth * 6 then 
                self.dx = 0
                self.x = self.map.mapWidthPixels - self.map.tileWidth * 6
            elseif self.x > self.map.mapWidthPixels - self.map.tileWidth * 4 then
                self.dx = -WALKING_SPEED
                self.animation = self.animations['walking']
            elseif self.x < self.map.mapWidthPixels - self.map.tileWidth * 6 then
                self.dx = WALKING_SPEED
                self.animation = self.animations['walking']
            end
        end
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY and 
            ...
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= TILE_EMPTY and
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= CLOUD_LEFT and
            self.map:tileAt(self.x + self.width - 1, self.y).id ~= CLOUD_RIGHT then
            -- reset y velocity
            self.dy = 0
